  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {unread > 0 && (
          <Button variant="outline" size="sm" onClick={handleMarkAll} className="self-start">
          ...
          {notifs.map((n) => (
            <Card key={n.id} className={n.isRead ? "opacity-60" : ""}>
              <CardContent className="py-3 px-4 flex flex-col sm:flex-row sm:items-start gap-3">
                <div className={`mt-0.5 text-xs font-medium px-2 py-0.5 rounded-full shrink-0 self-start ${typeColors[n.type ?? ""] ?? "bg-gray-100 text-gray-800"}`}>
